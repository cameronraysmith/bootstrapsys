...
clone_repos() {
    ...
    if [[ ! $nopull ]]; then
        ...
        if [[ -d "$BASE/bootstrapsys/.git" ]]; then
            output "Pulling bootstrapsys"
            cd "$BASE/bootstrapsys"
            git pull
        else
            output "Cloning bootstrapsys"
            if [[ -d "$BASE/bootstrapsys" ]]; then
                output "Creating backup for existing bootstrapsys"
                mv "$BASE/bootstrapsys" "${BASE}/bootstrapsys.bak.$$"
            fi
            git clone https://github.com/cameronraysmith/bootstrapsys.git
        fi
    fi
}
...
set_base_default() {  # if PROJECT_HOME not set
    ...
    if [[ "x$this_repo" = "xbootstrapsys.git" ]]; then
...
# if [[ -r $BASE/.ruby-version ]]; then
#   RUBY_VER=`cat $BASE/.ruby-version`
# fi
...
if [[ ! $quiet ]]; then
  ...
  This script will setup a local bootstrapsys environment, this
  includes
...
# Clone bootstrapsys repositories
...
# # Install ruby gems
# #rvm --create 1.9.3@devset
# #rvm gemset import $BASE/requirements/bootstrapsys/devset
# #rvm use 1.9.3@devset
# rvm --create 1.9.3@testset
# rvm gemset import $BASE/requirements/bootstrapsys/testset
# rvm use 1.9.3@testset
...
# Create bootstrapsys virtualenv and link it to repo
# virtualenvwrapper automatically sources the activation script
if [[ $systempkgs ]]; then
    mkvirtualenv -q -a "$WORKON_HOME" --system-site-packages bootstrapsys || {
    ...
    # --no-site-packages
    mkvirtualenv -q -a "$WORKON_HOME" bootstrapsys || {
      error "mkvirtualenv exited with a non-zero error"
      return 1
    }
fi
...
# SITE_PACKAGES="$WORKON_HOME/bootstrapsys/lib/python2.7/site-packages"
# cd "$SITE_PACKAGES"
...
output "Installing bootstrapsys Python requirements"
#pip install -r $BASE/bootstrapsys/requirements/bootstrapsys/devset.eggs
pip install -r $BASE/requirements/bootstrapsys/testset.eggs
